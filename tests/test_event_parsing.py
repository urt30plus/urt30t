...
from urt30t.events import LogEntry
from urt30t.models import BombAction, FlagAction, HitLocation, HitMode, KillMode


def test_event_account_kick() -> None:
    log_entry = LogEntry(
        kind=events.AccountKick, data="13 - [ABC]foobar^7 rejected: no account"
    )
    e = events.AccountKick.from_log_entry(log_entry)
    assert e.slot == "13"
    assert e.text == "[ABC]foobar^7 rejected: no account"


def test_event_account_rejected() -> None:
    log_entry = LogEntry(kind=events.AccountRejected, data='19 -  - "no account"')
    e = events.AccountRejected.from_log_entry(log_entry)
    assert e.slot == "19"
    assert e.text == '-  - "no account"'


def test_event_account_validated() -> None:
    log_entry = LogEntry(kind=events.AccountValidated, data='0 - m0neysh0t - 6 - ""')
    e = events.AccountValidated.from_log_entry(log_entry)
    assert e.slot == "0"
...
def test_event_assist() -> None:
    ...
    e = events.Assist.from_log_entry(log_entry)
    assert e.slot == "12"
    assert e.killer == "1"
    assert e.victim == "0"
...
def test_event_bomb_defused() -> None:
    log_entry = LogEntry(events.Bomb, data="defused by 11!")
    e = events.Bomb.from_log_entry(log_entry)
    assert e.slot == "11"
    assert e.action is BombAction.DEFUSED


def test_event_bomb_tossed() -> None:
    log_entry = LogEntry(events.Bomb, data="tossed by 8")
    e = events.Bomb.from_log_entry(log_entry)
    assert e.slot == "8"
...
def test_event_bomb_holder() -> None:
    log_entry = LogEntry(events.BombHolder, data="2")
    e = events.BombHolder.from_log_entry(log_entry)
    assert e.slot == "2"


def test_event_client_begin() -> None:
    log_entry = LogEntry(events.ClientBegin, data="4")
    e = events.ClientBegin.from_log_entry(log_entry)
    assert e.slot == "4"


def test_event_client_connect() -> None:
    log_entry = LogEntry(events.ClientConnect, data="15")
    e = events.ClientBegin.from_log_entry(log_entry)
    assert e.slot == "15"


def test_event_client_disconnect() -> None:
    log_entry = LogEntry(events.ClientDisconnect, data="15")
    e = events.ClientDisconnect.from_log_entry(log_entry)
    assert e.slot == "15"


def test_event_client_melted() -> None:
    log_entry = LogEntry(events.ClientMelted, data="15")
    e = events.ClientMelted.from_log_entry(log_entry)
    assert e.slot == "15"


def test_event_client_spawn() -> None:
    log_entry = LogEntry(events.ClientSpawn, data="15")
    e = events.ClientSpawn.from_log_entry(log_entry)
    assert e.slot == "15"


def test_event_flag() -> None:
    log_entry = LogEntry(events.Flag, data="0 2: team_CTF_redflag")
    e = events.Flag.from_log_entry(log_entry)
    assert e.slot == "0"
...
def test_event_flag_capture_time() -> None:
    log_entry = LogEntry(events.FlagCaptureTime, data="0: 15750")
    e = events.FlagCaptureTime.from_log_entry(log_entry)
    assert e.slot == "0"
...
def test_event_freeze() -> None:
    ...
    e = events.Freeze.from_log_entry(log_entry)
    assert e.slot == "4"
    assert e.target == "17"
...
def test_event_hit() -> None:
    ...
    e = events.Hit.from_log_entry(log_entry)
    assert e.slot == "4"
    assert e.attacker == "8"
...
def test_event_init_auth() -> None:
    ...
    e = events.InitAuth.from_log_entry(log_entry)
...
def test_event_kill() -> None:
    ...
    e = events.Kill.from_log_entry(log_entry)
    assert e.slot == "8"
    assert e.victim == "5"
...
def test_event_survivor_winner_player() -> None:
    log_entry = LogEntry(events.SurvivorWinner, data="2")
    e = events.SurvivorWinner.from_log_entry(log_entry)
    assert e.slot == "2"
    assert e.team is None


def test_event_survivor_winner_team() -> None:
    log_entry = LogEntry(events.SurvivorWinner, data="Red")
    e = events.SurvivorWinner.from_log_entry(log_entry)
...
def test_event_team_scores() -> None:
    log_entry = LogEntry(events.TeamScores, data="red:8  blue:5")
    e = events.TeamScores.from_log_entry(log_entry)
    assert e.red == 8
    assert e.blue == 5
...
def test_event_thaw_out_finished() -> None:
    ...
    e = events.ThawOutFinished.from_log_entry(log_entry)
    assert e.slot == "4"
    assert e.target == "13"
...
def test_event_thaw_out_started() -> None:
    ...
    e = events.ThawOutStarted.from_log_entry(log_entry)
    assert e.slot == "4"
    assert e.target == "9"
